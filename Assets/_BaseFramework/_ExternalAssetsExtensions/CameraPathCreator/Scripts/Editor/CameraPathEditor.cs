    ...
    public class CameraPathEditor : Editor
    {
        ...
        protected virtual void DrawNextHandle(int i)
        {
            if (i < cameraPath.pointList.Count - 1)
            {
                ...
                if (handlePositionMode == CPC_EManipulationModes.Free)
                {
                    ...
                    var fmh_570_97_638613611676667022 = Quaternion.identity; var fmh_564_182_638677342578930371 = Quaternion.identity; posNext = Handles.FreeMoveHandle(cameraPath.pointList[i].position + cameraPath.pointList[i].handlenext, size, Vector3.zero, Handles.SphereHandleCap);
#else
        ...
        protected virtual void DrawPrevHandle(int i)
        {
            if (i > 0)
            {
                ...
                {
#if UNITY_5_5_OR_NEWER
                    var fmh_622_97_638613611676693016 = Quaternion.identity; var fmh_615_182_638677342579365352 = Quaternion.identity; posPrev = Handles.FreeMoveHandle(cameraPath.pointList[i].position + cameraPath.pointList[i].handleprev, 0.1f * HandleUtility.GetHandleSize(cameraPath.pointList[i].position + cameraPath.pointList[i].handleprev), Vector3.zero, Handles.SphereHandleCap);
#else
